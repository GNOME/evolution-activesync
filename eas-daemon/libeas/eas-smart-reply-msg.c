...
struct _EasSmartReplyMsgPrivate {
	gchar *account_id;
	gchar *client_id;
...
G_DEFINE_TYPE_WITH_PRIVATE (EasSmartReplyMsg, eas_smart_reply_msg, EAS_TYPE_MSG_BASE);

static void
eas_smart_reply_msg_init (EasSmartReplyMsg *object)
{
	EasSmartReplyMsgPrivate *priv;
	object->priv = priv = eas_smart_reply_msg_get_instance_private (object);
...
static void
eas_smart_reply_msg_finalize (GObject *object)
{
	EasSmartReplyMsg *msg = (EasSmartReplyMsg *) object;
	EasSmartReplyMsgPrivate *priv = msg->priv;
	...
	G_OBJECT_CLASS (eas_smart_reply_msg_parent_class)->finalize (object);
}

static void
eas_smart_reply_msg_class_init (EasSmartReplyMsgClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	object_class->finalize = eas_smart_reply_msg_finalize;
}

EasSmartReplyMsg *
eas_smart_reply_msg_new (const gchar *account_id,
			 const gchar *client_id,
			 const gchar *source_folder_id,
			 const gchar *source_item_id,
			 gchar *mime_string)
{
	EasSmartReplyMsg *msg = g_object_new (EAS_TYPE_SMART_REPLY_MSG, NULL);
	EasSmartReplyMsgPrivate *priv = msg->priv;
...
xmlDoc *
eas_smart_reply_msg_build_message (EasSmartReplyMsg *self)
{
	EasSmartReplyMsgPrivate *priv = self->priv;
	xmlDoc *doc = NULL;
	xmlNode *root = NULL;
	xmlNode *source = NULL;
	...
	root = xmlNewDocNode (doc, NULL, (xmlChar *) "SmartReply", NULL);
...
gboolean
eas_smart_reply_msg_parse_response (EasSmartReplyMsg *self,
				    xmlDoc *doc,
				    GError **error)
{
	(void) self;
